...
filter ::
  ...
  -> List a
  -> List a
filter f list = foldRight(\a b -> if (f a) then a :. b else b)(Nil)(list)
...
(++) ::
  ...
  -> List a
  -> List a
(++) list1 list2 = foldRight(\a b -> a :. b)(list2)(list1)
...
flatMap ::
  ...
  -> List a
  -> List b
flatMap f list = flatten (foldRight(\a b -> (f a) :. b)(Nil)(list))  -- foldRight(\a b -> (f a) :. b)(Nil)(list)
